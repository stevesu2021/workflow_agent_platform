...
class 可研评审Agent:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        # Add nodes
    "step_1": AgentNode(name="step_1", description="系统初始化
- **功能描述**：初始化智能体，加载配置和资源
- **输入数据**：无
- **处理逻辑**：
  1. 加载系统提示词
...
- **提示词模板**：
  ...
  ```")
    "step_2": AgentNode(name="step_2", description="规则提取与索引构建
...
- **提示词模板**：
  ...
  ```")
    "step_3": AgentNode(name="step_3", description="PDF内容提取
...
- **提示词模板**：
  ...
  ```")
    "step_4": AgentNode(name="step_4", description="规则-内容语义匹配
...
- **提示词模板**：
  ...
  ```")
    "step_5": AgentNode(name="step_5", description="单规则评审（循环执行）
- **功能描述**：对单条规则进行详细评审
- **输入数据**：规则详情、匹配的PDF内容
- **处理逻辑**：
  1. 分析规则类型（技术、安全、经济等）
...
- **提示词模板**：
  ...
  4. 给出评审结果（通过/不通过/需补充）
  ```")
    "step_6": AgentNode(name="step_6", description="评审结果汇总
...
- **提示词模板**：
  ...
  4. 最终评审结论
  ```")
    "step_7": AgentNode(name="step_7", description="输出格式化
